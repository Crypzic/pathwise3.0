...
import { askSystemPrompt, clamp01, extractTopicsPrompt, validateBreakdown, validateGrade, validateTopics, validateQuestions, validateVerdict, validateWrittenQuestions, UNTRUSTED_INPUT_RULE, socraticSystemPrompt, } from "./prompts.js";
...
test("validateTopics dedupes case-insensitively and drops junk names", () => {
    ...
    assert.deepEqual(value.map((t) => t.name), ["Mitosis", "Meiosis"]);
        ...
        topics: [
            {
                ...
                objectives: ["  Explain   light reactions ", "", 42],
...
test("validateVerdict fails open to clean on unknown verdicts", () => {
    ...
    assert.equal(validateVerdict({ verdict: "banana", reason: "r" }).verdict, "clean");
...
test("validateGrade accepts real verdicts and fails kind on junk", () => {
    ...
    const junk = validateGrade({ verdict: "banana" });
