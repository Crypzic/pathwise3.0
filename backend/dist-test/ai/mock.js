...
function candidatePhrases(text) {
    ...
    const headingish = lines.filter((l) => /^(chapter|topic|unit|week|module|\d+[.)]|[-*•])/i.test(l));
...
export class MockAIProvider {
    ...
    _documentInput) {
        ...
        const topics = base.map((name, i) => {
            const occurrences = (materialText.match(new RegExp(escapeRegExp(name), "gi")) || []).length ||
                1;
    ...
    async generateQuiz(courseName, topics, count) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const correct = i % 4;
            const options = [0, 1, 2, 3].map((n) => n === correct
    ...
    async socraticReply(courseName, topicName, history, ctx) {
        ...
        if (ctx?.escalation === 2) {
            return wrap(`${probe}Let's shrink this to the very first step${focus}: what is the one term in the question you could look up or restate right now?`);
        }
        if (ctx?.escalation === 1) {
            return wrap(`${probe}Here's a more concrete nudge${focus}: focus on the part just before where you got lost. What changes at that point?`);
        }
        // Never a direct answer — always a guiding question.
        return wrap(`${probe}What do you already know${focus} that might point you toward the answer? What would happen if you tried the simplest case first?`);
    }
    async transcribeImage(courseName, image) {
        ...
        const lines = themes.map((t, i) => `${i + 1}. ${t}\n- Main idea of ${t.toLowerCase()} for ${courseName}\n- Detail worth remembering about ${t.toLowerCase()}`);
        return wrap(`Mock transcription (${image.mimeType}, ${image.data.length} bytes)\n${lines.join("\n")}`);
    }
    async classifyMaterial(courseName, materialText) {
        ...
        return wrap({ verdict: "clean", reason: "Looks like course material." });
    }
    async evaluateCommunityPost(communityName, title, body) {
        ...
        return wrap({ verdict: "clean", reason: "Looks fine for the community." });
    }
        ...
        const sections = [
            {
                ...
                body: `${topicName} is the focus of this breakdown. In plain terms, it covers the ideas your material emphasizes here. ` +
    ...
    async gradeWrittenAnswer(question, referenceAnswer, studentAnswer) {
        ...
        const explanation = verdict === "correct"
