...
import { detectAnswerLeak, detectExtractionAttempt, fallbackProbe, retryNudge, } from "../lib/socraticGuard.js";
...
function openingTurn(topicName, contextNote) {
    if (contextNote) {
 ...
 */
async function guardedReply(userId, courseName, topicName, history, turnIndex, ctx) {
    ...
    const second = await socraticReply(userId, courseName, topicName, retryHistory, ctx);
...
export default async function socraticRoutes(app) {
    ...
    app.post("/api/socratic/sessions/:id/messages", {
        ...
        if (session.turnCount >= maxTurns) {
            return reply.code(409).send({
                ...
                message: "This has been a long session — end it to bank your progress, then start a fresh one.",
            });
        }
        ...
        let result;
        try {
            result = await guardedReply(req.user.sub, session.course.name, session.topic?.name ?? null, history, session.turnCount, ctx);
        }
        catch (err) {
    ...
    app.post("/api/socratic/sessions/:id/end", { preHandler: [app.authenticate] }, async (req, reply) => {
        const { id } = req.params;
        const session = await prisma.socraticSession.findFirst({
            where: { id, userId: req.user.sub },
        });
        ...
        if (meaningful && session.topicId) {
            const recorded = await recordSocraticDepth(req.user.sub, session.topicId, session.turnCount);
            mastery = {
        ...
        const newBadges = [];
        if (meaningful) {
            const award = await awardXp(req.user.sub, XP.socraticSession, "socratic_session");
    ...
    app.post("/api/socratic/intro-seen", { preHandler: [app.authenticate] }, async (req, reply) => {
