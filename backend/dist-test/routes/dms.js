...
async function isBlockedEitherWay(x, y) {
...
export default async function dmRoutes(app) {
    ...
    app.get("/api/dms", { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    app.get("/api/dms/:id", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await isGuestUser(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
        const me = req.user.sub;
    ...
    app.post("/api/dms/:id/respond", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await isGuestUser(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
        const me = req.user.sub;
    ...
    app.post("/api/dms/:id/mute", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await isGuestUser(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
        const me = req.user.sub;
    ...
    app.post("/api/dms/block", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await isGuestUser(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
        const me = req.user.sub;
    ...
    app.post("/api/dms/messages/:messageId/report", {
        preHandler: [app.authenticate],
