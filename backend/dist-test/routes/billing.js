...
import { billing, PLANS, activatePremium, revertToFree, entitlementsFor, } from "../lib/billing.js";
...
export default async function billingRoutes(app) {
    ...
    app.post("/api/billing/checkout", { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        try {
            const session = await billing.createCheckout(user.id, user.email, parsed.data.interval);
    ...
    app.post("/api/billing/mock/complete", { preHandler: [app.authenticate] }, async (req, reply) => {
