...
export default async function studyPlanRoutes(app) {
    // Step 6 — today's quests, confidence gauge, quest path, drop banner.
    app.get("/api/courses/:id/study-plan", { preHandler: [app.authenticate] }, async (req, reply) => {
        const { id } = req.params;
    ...
    app.get("/api/courses/:id/dashboard", { preHandler: [app.authenticate] }, async (req, reply) => {
        const { id } = req.params;
        ...
        const understanding = assessed.filter((t) => t.mastery >= 0.5 && t.guessSignal <= 0.45);
        const guessing = assessed.filter((t) => t.mastery >= 0.5 && t.guessSignal > 0.45);
