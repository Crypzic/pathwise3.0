...
export default async function profileRoutes(app) {
        ...
        return reply.send({
            ...
            companion: user.companion
                ? {
                    ...
                    equipped: JSON.parse(user.companion.equippedJson),
                }
                : null,
        });
    });
    ...
    app.post("/api/profile/delete", { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        const now = new Date();
        const purgeAfter = new Date(now.getTime() + RECOVERY_WINDOW_DAYS * 24 * 60 * 60 * 1000);
        await prisma.user.update({
            where: { id: user.id },
    ...
    app.post("/api/profile/restore", { config: { rateLimit: { max: 5, timeWindow: "10 minutes" } } }, async (req, reply) => {
        const parsed = z
    ...
    app.get("/api/profile/export", { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        reply.header("Content-Disposition", `attachment; filename="pathwise-export-${user.id}.json"`);
