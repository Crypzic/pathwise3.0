...
function publicUser(u, isPremium = false) {
    return {
        id: u.id,
        name: u.name,
        email: u.email,
...
export default async function authRoutes(app) {
    ...
    app.post("/api/auth/guest", { config: { rateLimit: { max: 5, timeWindow: "10 minutes" } } }, async (req, reply) => {
        ...
        const token = app.jwt.sign({ sub: user.id, email: user.email }, { expiresIn: `${env.GUEST_TTL_DAYS}d` });
        return reply.code(201).send({ token, user: publicUser(user) });
    });
    ...
    app.post("/api/auth/signin", { config: { rateLimit: { max: 10, timeWindow: "10 minutes" } } }, async (req, reply) => {
    ...
    app.post("/api/auth/accept-privacy", { preHandler: [app.authenticate] }, async (req, reply) => {
        const user = await prisma.user.update({
            where: { id: req.user.sub },
            data: {
    ...
    app.post("/api/auth/forgot-password", { config: { rateLimit: { max: 5, timeWindow: "10 minutes" } } }, async (req, reply) => {
