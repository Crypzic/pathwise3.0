...
import { ACADEMIC_LEVELS, AVAILABILITY, CONTENT_PREFS, STUDY_STYLES, isAcademicLevel, isStudyStyle, normalizeBuddyPrefs, normalizeContentPrefs, normalizeFreeList, parseStoredList, } from "../lib/onboardingModel.js";
...
function shaped(profile) {
    return {
        ...
        buddyPrefs: normalizeBuddyPrefs(profile ? parseStoredObject(profile.buddyPrefsJson || "{}") : {}),
        onboarded: profile?.onboardedAt != null,
    };
}
export default async function onboardingRoutes(app) {
    app.get("/api/onboarding", { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    app.put("/api/onboarding", { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        if (d.contentPrefs !== undefined)
            data.contentPrefsJson = JSON.stringify(normalizeContentPrefs(d.contentPrefs));
        if (d.communityInterests !== undefined)
            data.communityInterestsJson = JSON.stringify(normalizeFreeList(d.communityInterests));
        if (d.studyStyle !== undefined)
            data.studyStyle = d.studyStyle || null;
        if (d.buddyPrefs !== undefined) {
            ...
            const stored = normalizeBuddyPrefs(existing ? parseStoredObject(existing.buddyPrefsJson || "{}") : {});
            data.buddyPrefsJson = JSON.stringify(normalizeBuddyPrefs({
