...
function publicItem(item, total) {
    return {
        id: item.id,
...
export default async function quizRoutes(app) {
    ...
    app.post("/api/quiz/sessions", { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : "Couldn't start a quiz.";
    ...
    app.post("/api/quiz/sessions/:id/answer", { preHandler: [app.authenticate] }, async (req, reply) => {
        const { id } = req.params;
        ...
        if (item.kind === "written") {
            ...
            let grade;
            try {
                grade = await gradeWrittenAnswer(req.user.sub, item.question, item.referenceAnswer ?? item.explanation, answerText);
            }
            catch (err) {
                if (err instanceof AIBudgetExceededError) {
                    return reply.code(429).send({ error: err.message });
        ...
        const award = await awardXp(req.user.sub, isCorrect ? XP.quizCorrect : XP.quizIncorrect, isCorrect ? "quiz_correct" : "quiz_incorrect");
        ...
        const remaining = session.items.filter((i) => i.answeredAt === null && i.id !== item.id).length;
        let completion = null;
        if (remaining === 0) {
            ...
            const bonus = session.kind === "review" ? XP.reviewCompleted : XP.quizCompleted;
            const bonusAward = await awardXp(req.user.sub, bonus, session.kind === "review" ? "review_completed" : "quiz_completed");
    ...
    app.get("/api/quiz/sessions/:id/review", { preHandler: [app.authenticate] }, async (req, reply) => {
        const { id } = req.params;
        const session = await prisma.quizSession.findFirst({
        ...
        return reply.send({
            cards: session.items.map((i) => {
                const options = JSON.parse(i.optionsJson);
                return {
                    id: i.id,
    ...
    app.post("/api/quiz/sessions/:id/abandon", { preHandler: [app.authenticate] }, async (req, reply) => {
        const { id } = req.params;
        const session = await prisma.quizSession.findFirst({
