...
function partnerName(u) {
    return u.username ?? u.name;
}
function shapedMessage(m, me) {
    return {
        id: m.id,
        role: m.role,
        mine: m.senderId === me,
...
export default async function roomRoutes(app) {
    ...
    // My rooms.
    app.get("/api/rooms", { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    app.get("/api/rooms/:id", { preHandler: [app.authenticate] }, async (req, reply) => {
        const me = req.user.sub;
        const { id } = req.params;
        ...
        const partnerHelpAt = myHelpField === "helpAAt" ? room.helpBAt : room.helpAAt;
        return reply.send({
            room: {
                id: room.id,
    ...
    app.post("/api/rooms/:id/messages", {
        preHandler: [app.authenticate],
    ...
    app.post("/api/rooms/:id/topic", { preHandler: [app.authenticate] }, async (req, reply) => {
        const me = req.user.sub;
        const { id } = req.params;
        const parsed = z
    ...
    app.post("/api/rooms/:id/help", {
        ...
        if (!bothWantHelp(updated.helpAAt, updated.helpBAt, now)) {
            return reply.send({
                facilitated: false,
                message: "Noted — PATHWISE joins in when your buddy asks for help too.",
            });
        }
        ...
        let text;
        try {
            const raw = await socraticReply(me, "Study room", updated.topicName, history, {
                grounding: "You are facilitating TWO students studying together" +
    ...
    app.post("/api/rooms/:id/end", { preHandler: [app.authenticate] }, async (req, reply) => {
        const me = req.user.sub;
        const { id } = req.params;
        const room = await prisma.studyRoom.findFirst({
            where: { id, status: "active", OR: [{ aId: me }, { bId: me }] },
