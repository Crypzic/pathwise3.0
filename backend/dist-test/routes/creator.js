...
import { canPublish, cleanText, moderationFromVerdict, videoKindFor, } from "../lib/creatorModel.js";
...
function isInternalTester(req) {
    return opsAuthorized(req.headers["x-cron-secret"]);
}
function creatorSurfaceOpen(req) {
...
export default async function creatorRoutes(app) {
    ...
    app.post("/api/creator/videos", {
        ...
        let subject = null;
        try {
            const verdict = await classifyMaterial(req.user.sub, "creator-video", analysisText);
            const lane = moderationFromVerdict(verdict.verdict);
            status = lane.status;
            moderationStatus = lane.moderationStatus;
            if (status !== "rejected") {
                const extracted = await extractTopics(req.user.sub, title, analysisText);
    ...
    app.get("/api/creator/videos", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (!creatorSurfaceOpen(req)) {
            return reply.code(404).send({ error: "Not found" });
        }
    ...
    app.post("/api/creator/videos/:id/publish", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (!creatorSurfaceOpen(req)) {
            return reply.code(404).send({ error: "Not found" });
        }
        const { id } = req.params;
        ...
        const verdict = canPublish(video.status, video.moderationStatus);
        if (!verdict.ok) {
    ...
    app.delete("/api/creator/videos/:id", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (!creatorSurfaceOpen(req)) {
            return reply.code(404).send({ error: "Not found" });
        }
        const { id } = req.params;
    ...
    app.get("/api/creator/feed", { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    app.post("/api/creator/videos/:id/interact", {
        preHandler: [app.authenticate],
    ...
    app.post("/api/creator/videos/:id/comments", {
        preHandler: [app.authenticate],
    ...
    app.post("/api/creator/follow", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (!features.userVideoPosting) {
            return reply.code(404).send({ error: "Not found" });
        }
        if (await isGuestUser(req.user.sub)) {
