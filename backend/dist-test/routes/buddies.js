...
const GUEST_MESSAGE = "Study buddies need an account — create one to keep your identity.";
const requestSchema = z.object({
    toId: z.string().min(1),
    message: z.string().max(500).optional(),
});
export default async function buddyRoutes(app) {
    // Suggested matches (empty + a flag while not discoverable).
    app.get("/api/buddies/matches", { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    app.post("/api/buddies/discoverable", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await isGuestUser(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
    ...
    app.get("/api/buddies/requests", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await isGuestUser(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
    ...
    app.post("/api/buddies/requests/:id/respond", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await isGuestUser(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
        const { id } = req.params;
    ...
    app.get("/api/buddies", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await isGuestUser(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
