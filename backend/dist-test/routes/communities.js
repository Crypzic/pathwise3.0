...
import { POST_KINDS, REPORT_REASONS, screenText, } from "../lib/communityModel.js";
...
const GUEST_MESSAGE = "Communities need an account — create one to keep your identity and join the conversation.";
...
export default async function communityRoutes(app) {
    ...
    async function rejectGuests(userId) {
        return isGuestUser(userId);
    }
    // The subject tree with member counts and this user's memberships.
    app.get("/api/communities", { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    app.post("/api/communities/:id/leave", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await rejectGuests(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
        const { id } = req.params;
    ...
    app.get("/api/communities/:id", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await rejectGuests(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
        const { id } = req.params;
    ...
    app.post("/api/communities/:id/posts", {
        ...
        try {
            ...
            const verdict = await evaluateCommunityPost(req.user.sub, community?.name ?? "General", parsed.data.title, parsed.data.body);
    ...
    app.get("/api/communities/posts/:postId", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await rejectGuests(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
    ...
    app.post("/api/communities/posts/:postId/replies", {
        preHandler: [app.authenticate],
    ...
    app.post("/api/communities/posts/:postId/react", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await rejectGuests(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
        const { postId } = req.params;
    ...
    app.post("/api/communities/replies/:replyId/react", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (await rejectGuests(req.user.sub)) {
            return reply.code(403).send({ error: GUEST_MESSAGE });
        }
    ...
    app.delete("/api/communities/posts/:postId", { preHandler: [app.authenticate] }, async (req, reply) => {
        const { postId } = req.params;
        const post = await prisma.communityPost.findFirst({
    ...
    app.delete("/api/communities/replies/:replyId", { preHandler: [app.authenticate] }, async (req, reply) => {
        const { replyId } = req.params;
        const row = await prisma.communityReply.findFirst({
    ...
    app.post("/api/communities/reports", {
        preHandler: [app.authenticate],
