...
export default async function videoRoutes(app) {
    ...
    app.get("/api/fyp", { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        const liked = new Set(engagements.filter((e) => e.kind === "like").map((e) => e.videoId));
        const saved = new Set(engagements.filter((e) => e.kind === "save").map((e) => e.videoId));
        return reply.send({
    ...
    app.get("/api/videos", { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    app.post("/api/ops/videos", async (req, reply) => {
        ...
        const video = await prisma.curatedVideo.create({
            data: {
                ...
                difficulty: typeof body.difficulty === "number"
