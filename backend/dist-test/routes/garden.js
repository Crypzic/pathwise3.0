...
import { getCompanion, leafMatchReward, LEAF_MATCH_PAIRS, } from "../lib/garden.js";
...
export default async function gardenRoutes(app) {
    ...
    app.post("/api/garden/minigame/finish", {
    ...
    app.post("/api/garden/shop/purchase", { preHandler: [app.authenticate] }, async (req, reply) => {
        if (!features.leafMatch) {
            return reply.code(403).send(FROZEN_REPLY);
        }
        ...
        const updated = await prisma.user.update({
            ...
            data: {
                ...
                streakFreezes: item.key === "streak_freeze" ? { increment: 1 } : undefined,
            },
