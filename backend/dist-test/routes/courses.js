...
import { topicsWithMastery, courseMasteryPct, isDue, } from "../lib/mastery.js";
...
export default async function courseRoutes(app) {
    ...
    app.get("/api/courses", { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        const shaped = await Promise.all(courses.map(async (c) => {
    ...
    app.post("/api/courses/:id/uploads", {
        ...
        if (!kind && !imageKind) {
            return reply.code(415).send({
                error: "Only PDF, DOCX, PPTX, or an image (PNG, JPG, WebP) of your notes is supported.",
            });
        }
    ...
    app.delete("/api/courses/:id/uploads/:uploadId", { preHandler: [app.authenticate] }, async (req, reply) => {
