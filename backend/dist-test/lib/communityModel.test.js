...
import { countLinks, DEFAULT_COMMUNITIES, screenText, slugify, } from "./communityModel.js";
...
test("countLinks finds urls in both forms", () => {
    ...
    assert.equal(countLinks("see https://a.com and http://b.com plus www.c.com"), 3);
});
...
test("screenText blocks obvious junk", () => {
    ...
    assert.equal(screenText("   ").ok, false);
    assert.equal(screenText("buy now https://a.com https://b.com https://c.com https://d.com").ok, false);
    assert.equal(screenText("a".repeat(40)).ok, false);
    assert.equal(screenText("THIS IS ALL CAPS SHOUTING ABOUT NOTHING IN PARTICULAR AT ALL").ok, false);
});
