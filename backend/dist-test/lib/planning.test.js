...
import { allocateQuestions, buildQuests, computeConfidence, confidenceAlert, tierFor, topicPriority, } from "./planning.js";
...
test("allocation always lands on exactly the requested question count", () => {
    const topics = Array.from({ length: 9 }, (_, i) => topic({ id: `t${i}`, name: `T${i}`, weight: 0.2 + i * 0.08, mastery: i / 10 }));
    for (const count of [1, 2, 3, 5, 8, 12, 20]) {
        ...
        assert.ok(alloc.every((a) => a.questions >= 1), "no selected topic should get zero questions");
    }
});
...
test("confidence rises with mastery", () => {
    const mk = (mastery) => computeConfidence({
        topics: [
...
test("mastering one topic out of ten doesn't read as exam ready", () => {
    const topics = [
        ...
        ...Array.from({ length: 9 }, (_, i) => topic({ id: `unknown${i}`, mastery: 0, weight: 0.8, attemptCount: 0 })),
    ];
...
test("a fully-learned, fully-covered course reaches the top tier", () => {
    const topics = Array.from({ length: 6 }, (_, i) => topic({
        id: `t${i}`,
        mastery: 0.95,
        weight: 0.8,
        attemptCount: 8,
...
test("the plan never returns more than four quests and never none", () => {
    const many = Array.from({ length: 20 }, (_, i) => topic({ id: `t${i}`, name: `T${i}`, mastery: 0.1, dueAt: PAST, attemptCount: 3 }));
    assert.ok(buildQuests(many, NOW).length <= 4);
    // A fully-mastered course still gets something to do.
    const mastered = Array.from({ length: 4 }, (_, i) => topic({
        id: `m${i}`,
        mastery: 1,
        dueAt: FUTURE,
        attemptCount: 9,
...
test("quest ids are unique so React keys don't collide", () => {
    const topics = Array.from({ length: 6 }, (_, i) => topic({ id: `t${i}`, name: `T${i}`, mastery: i / 10, dueAt: PAST, attemptCount: 3 }));
