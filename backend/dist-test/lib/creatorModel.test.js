...
import { canPublish, cleanText, moderationFromVerdict, videoKindFor, } from "./creatorModel.js";
...
test("publish requires analysis done AND an approved moderation pass", () => {
    ...
    assert.match(canPublish("pending_review", "flagged").reason ?? "", /moderation review/i);
    assert.match(canPublish("rejected", "rejected").reason ?? "", /rejected/i);
