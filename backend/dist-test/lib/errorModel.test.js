...
import { boundReport, dayKeyOf, fingerprintError, MAX_MESSAGE, MAX_STACK, } from "./errorModel.js";
test("the same error with different ids/lines shares a fingerprint", () => {
    const a = fingerprintError('User "cku7x9aab0001" not found', "Error: nope\n    at findUser (src/lib/users.ts:41:7)");
    const b = fingerprintError('User "cku8zzq1c0999" not found', "Error: nope\n    at findUser (src/lib/users.ts:97:13)");
    assert.equal(a, b);
});
