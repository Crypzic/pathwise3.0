 ...
 */
export async function grantBadge(userId, key) {
...
export async function awardXp(userId, amount, reason) {
    ...
    if (rankedUp) {
        ...
        const { notifyUnlock } = await import("./notifications.js");
        await notifyUnlock(userId, `New rank: ${afterRank.name}`, `You reached level ${afterRank.level}. New profile frames may be available.`);
    }
    return {
        xp: updated.xp,
        xpGained: amount,
