...
export async function collectUserStoragePaths(userId) {
 ...
 */
export async function removeUserStoredFiles(userId) {
    const paths = await collectUserStoragePaths(userId);
    for (const storagePath of paths) {
        await storage.remove(storagePath).catch((err) => {
            console.warn(`⚠️  storage sweep: failed to remove ${storagePath}:`, err instanceof Error ? err.message : err);
        });
    }
    return paths.length;
}
