...
import { detectAnswerLeak, detectExtractionAttempt, fallbackProbe, retryNudge, } from "./socraticGuard.js";
// --- Replies that must be blocked -----------------------------------------
const LEAKING_REPLIES = [
...
test("blocks a lecture even when it ends with a question", () => {
    const lecture = "Cellular respiration happens in stages. " +
...
test("every fallback probe passes its own guard", () => {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        assert.equal(detectAnswerLeak(probe).leaked, false, `fallback probe would itself be blocked: ${probe}`);
    }
});
...
for (const attempt of EXTRACTION_ATTEMPTS) {
    test(`flags an extraction attempt: "${attempt.slice(0, 40)}…"`, () => {
        assert.equal(detectExtractionAttempt(attempt), true, `should have been flagged: ${attempt}`);
    });
}
test("ordinary student messages are not flagged as extraction attempts", () => {
    ...
    for (const msg of ordinary) {
        assert.equal(detectExtractionAttempt(msg), false, `wrongly flagged as an extraction attempt: ${msg}`);
    }
});
// --- Retry nudges ---------------------------------------------------------
