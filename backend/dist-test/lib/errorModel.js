...
export function boundReport(input) {
    const message = typeof input.message === "string" ? input.message.trim().slice(0, MAX_MESSAGE) : "";
    if (message.length === 0)
        return null;
    return {
        message,
        stack: typeof input.stack === "string" && input.stack.length > 0
            ? input.stack.slice(0, MAX_STACK)
            : null,
        url: typeof input.url === "string" && input.url.length > 0
