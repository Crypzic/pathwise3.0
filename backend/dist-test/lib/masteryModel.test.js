...
import { applyAnswer, applySocraticDepth, courseMasteryPct, DEFAULT_STATE, guessLikelihood, isDue, nextDueAt, } from "./masteryModel.js";
...
test("gains shrink as mastery approaches 1", () => {
    ...
    assert.ok(early.mastery - 0.1 > late.mastery - 0.9, "a beginner should gain more per answer than someone near mastery");
});
test("a fast, shallow correct answer earns less credit than a considered one", () => {
    ...
    assert.ok(considered.mastery > guessy.mastery, "reasoned answers should be worth more mastery than lucky clicks");
    assert.ok(guessy.guessSignal > considered.guessSignal, "the guessing signal should rise on fast shallow answers");
});
...
test("a topic becomes due only once its interval has elapsed", () => {
    ...
    assert.equal(isDue(dueAt, new Date(reviewedAt.getTime() + 2 * 60 * 60 * 1000)), false, "not due two hours after review");
    assert.equal(isDue(dueAt, new Date(reviewedAt.getTime() + DAY + 1000)), true, "due once the interval has passed");
});
...
test("a wrong answer pulls the topic back for near-term review", () => {
    const learned = applyAnswer(applyAnswer(state(), { isCorrect: true, timeMs: 9000, socraticTurns: 2 }), { isCorrect: true, timeMs: 9000, socraticTurns: 2 });
...
test("Socratic depth alone can't reach mastery", () => {
    ...
    assert.ok(oneSession.mastery - 0.4 <= 0.12 + 1e-9, "a single session should not exceed the per-session cap");
});
// --- Course rollup ---------------------------------------------------------
