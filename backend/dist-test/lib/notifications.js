 ...
 */
export async function deliver(userId, notification, dayKey) {
    try {
...
export async function notifyUnlock(userId, title, body) {
    const user = await prisma.user.findUnique({
        where: { id: userId },
        select: { notifyUnlocks: true, timezone: true },
    });
    if (!user?.notifyUnlocks)
        return;
 ...
 */
async function emailCopy(to, notification) {
    try {
...
export async function runNotificationSweep(now = new Date()) {
    ...
    for (const user of users) {
        ...
        if (hour !== user.notifyHour && hour !== user.notifyHour + 1)
            continue;
