...
test("rejects malformed or empty headers", () => {
    for (const header of ["", "garbage", "t=", "v1=abc", "t=123", "t=123,v1="]) {
        assert.equal(verifyStripeSignature(BODY, header, SECRET, 300, NOW_MS), false, `should have rejected header: "${header}"`);
    }
});
test("rejects a non-numeric timestamp", () => {
    assert.equal(verifyStripeSignature(BODY, "t=notanumber,v1=deadbeef", SECRET, 300, NOW_MS), false);
});
