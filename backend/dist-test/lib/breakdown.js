 ...
 * read, not a re-parse.
 */
async function gatherMaterialText(courseId) {
    ...
    for (const upload of uploads) {
        if (combined.length >= MATERIAL_CAP)
            break;
 ...
 */
export async function getOrCreateBreakdown(userId, topicId) {
    ...
    if (topic.breakdownJson) {
        try {
            return {
                breakdown: JSON.parse(topic.breakdownJson),
                cached: true,
    ...
    const breakdown = await explainTopic(userId, course.name, topic.name, conceptContext, materialText);
    if (!breakdown)
        return null;
