...
import { applyAnswer, applySocraticDepth, DEFAULT_STATE, isDue, nextDueAt, } from "./masteryModel.js";
...
export { applyAnswer, applySocraticDepth, courseMasteryPct, isDue, nextDueAt, guessLikelihood, } from "./masteryModel.js";
/** Read a user's state for a topic, falling back to the default. */
export async function loadState(userId, topicId) {
    const row = await prisma.topicMastery.findUnique({
        where: { userId_topicId: { userId, topicId } },
    });
    if (!row)
        return { ...DEFAULT_STATE, socraticTurnCount: 0 };
    return {
        mastery: row.mastery,
 ...
 */
export async function recordQuizAnswer(userId, topicId, input) {
 ...
 */
export async function recordSocraticDepth(userId, topicId, turns) {
    const current = await loadState(userId, topicId);
 ...
 */
export async function topicsWithMastery(userId, courseId) {
