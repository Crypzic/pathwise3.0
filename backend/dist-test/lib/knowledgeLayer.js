...
import { formatConceptContext, toConceptView, } from "./knowledgeLayerModel.js";
export { formatConceptContext, toConceptView, difficultyLabel, } from "./knowledgeLayerModel.js";
/** Fetch one concept (ownership enforced via the course relation). */
export async function getConcept(userId, topicId) {
 ...
 */
export async function getConceptContext(userId, topicId) {
    const topic = await prisma.topic.findFirst({
        where: { id: topicId, knowledgeMap: { course: { userId } } },
    });
    if (!topic)
...
export async function listConcepts(userId, courseId) {
