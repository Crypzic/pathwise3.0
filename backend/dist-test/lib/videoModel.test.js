...
import { DEFAULT_VIDEOS, rankVideos, validateVideoInput, } from "./videoModel.js";
...
test("validateVideoInput enforces the required fields", () => {
    assert.equal(validateVideoInput({
        title: "A fine video",
        creator: "Someone",
    ...
    assert.match(validateVideoInput({ title: "abc", creator: "xy", subject: "Physics", url: "notaurl" }) ?? "", /url/);
});
test("seed catalog is well-formed with unique urls", () => {
    const urls = new Set();
    for (const s of DEFAULT_VIDEOS) {
        assert.equal(validateVideoInput(s), null, `seed invalid: ${s.title}`);
