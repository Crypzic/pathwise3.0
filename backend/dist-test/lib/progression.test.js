...
test("rank thresholds are monotonically increasing", () => {
    for (let i = 1; i < RANKS.length; i++) {
        assert.ok(RANKS[i].minXp > RANKS[i - 1].minXp, `rank ${i} threshold must exceed the previous one`);
    }
});
// --- Streaks ---------------------------------------------------------------
...
test("the first ever study day starts the streak at 1", () => {
    const r = nextStreak({ streakCount: 0, bestStreak: 0, lastActiveDay: null, streakFreezes: 0 }, "2026-03-10");
    assert.equal(r.streakCount, 1);
...
test("day arithmetic crosses month and year boundaries", () => {
    ...
    const newYear = nextStreak({ ...base, lastActiveDay: "2026-12-31" }, "2027-01-01");
