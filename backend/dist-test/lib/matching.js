...
import { normalizeBuddyPrefs, parseStoredList, } from "./onboardingModel.js";
import { rankMatches, } from "./matchModel.js";
export async function signalsFor(userId) {
    ...
    const courseTopics = user.courses.flatMap((c) => c.knowledgeMap?.topics.map((t) => t.name) ?? []);
    return {
        userId: user.id,
        ...
        buddyPrefs: normalizeBuddyPrefs(profile ? safeParse(profile.buddyPrefsJson) : {}),
    };
}
