...
export function imageKindFor(filename, mimeType) {
...
async function parsePptx(buffer) {
    ...
    const slidePaths = Object.keys(zip.files)
        ...
        .sort((a, b) => {
        const n = (p) => Number(p.match(/slide(\d+)\.xml/)?.[1] ?? 0);
        return n(a) - n(b);
    });
    const out = [];
    for (const path of slidePaths) {
        ...
        const paragraphs = [...xml.matchAll(/<a:p\b[\s\S]*?<\/a:p>/g)].map((m) => [...m[0].matchAll(/<a:t>(.*?)<\/a:t>/gs)]
