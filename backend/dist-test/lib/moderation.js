...
export async function moderateMaterial(userId, courseName, text) {
    ...
    if (trimmed.length < MIN_USABLE_CHARS) {
        return {
            verdict: "off_topic",
            reason: "We couldn't read enough text from this file to build a study map. If it's a scanned document, try a text-based PDF.",
            allowed: false,
        };
    }
    ...
    try {
        ...
        if (result.verdict === "off_topic") {
            return {
                verdict: "off_topic",
                reason: "This doesn't look like course material. Try uploading a syllabus, lecture slides, or reading notes.",
                allowed: false,
            };
        }
