...
import { isDue, courseMasteryPct } from "./masteryModel.js";
 ...
 * actually surfaces.
 */
export function topicPriority(t, now = new Date()) {
    const gap = 1 - t.mastery;
    let score = t.weight * (0.35 + 0.65 * gap);
    if (isDue(t.dueAt, now))
        score *= 1.5;
 ...
 */
export function allocateQuestions(topics, count, now = new Date()) {
    ...
    const alloc = pool.map((p) => ({
        topic: p.topic,
        questions: total > 0 ? Math.max(1, Math.round((p.score / total) * count)) : 1,
    }));
    ...
    while (sum > count) {
        ...
        if (idx === -1)
            break; // everything is at the 1-question floor
...
export function computeConfidence(input) {
    ...
    const score = Math.round(Math.max(0, Math.min(1, blended * coverageFactor)) * 100);
    return {
        score,
        tier: tierFor(score),
        components: {
 ...
 */
export function buildQuests(topics, now = new Date()) {
    if (topics.length === 0) {
        return [
            {
                id: "upload",
    ...
    const weakest = topics
        ...
        .sort((a, b) => a.mastery * (1 - a.weight) - b.mastery * (1 - b.weight))[0];
    if (weakest) {
        quests.push({
            id: `quiz:${weakest.id}`,
            kind: "quiz",
            title: `Quiz: ${weakest.name}`,
            subtitle: weakest.attemptCount === 0
    ...
    const fallbackDeepDive = topics
        .filter((t) => !used.has(t.id) && t.socraticTurnCount === 0 && t.weight >= 0.6)
 ...
 */
export function confidenceAlert(topics, now = new Date()) {
    const slipping = topics
        .filter((t) => t.attemptCount >= 2 &&
        t.mastery < 0.6 &&
...
export function pathState(t) {
    if (t.attemptCount === 0 && t.socraticTurnCount === 0)
        return "locked";
    if (t.mastery >= 0.8)
        return "mastered";
    if (t.mastery >= 0.45)
