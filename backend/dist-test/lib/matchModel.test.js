...
import { overlap, rankMatches, scoreMatch, } from "./matchModel.js";
...
test("overlap is case-insensitive and keeps first spelling", () => {
    assert.deepEqual(overlap(["Cell Biology", "genetics"], ["cell biology", "Physics"]), ["Cell Biology"]);
...
test("rankMatches excludes self and zero-score candidates, sorts desc", () => {
    ...
    assert.deepEqual(ranked.map((r) => r.candidate.userId), ["s", "w"]);
});
