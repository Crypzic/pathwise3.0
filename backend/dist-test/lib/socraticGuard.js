...
const PROSE_WALKTHROUGH = /\bfirst\b[\s\S]{20,}\b(?:then|next)\b[\s\S]{20,}\b(?:finally|lastly|and you get|which gives)\b/i;
/**
 ...
 */
export function detectAnswerLeak(reply) {
    ...
    for (const re of ANSWER_ANNOUNCERS) {
        if (re.test(text))
            return { leaked: true, reason: "announces_answer" };
    }
    for (const re of VERDICT_PATTERNS) {
        if (re.test(text))
            return { leaked: true, reason: "verdict_on_candidate" };
    }
...
export function detectExtractionAttempt(message) {
    const patterns = [
...
export function fallbackProbe(turnIndex) {
 ...
 */
export function retryNudge(reason) {
    switch (reason) {
