 ...
 * can pitch against.
 */
export function formatConceptContext(concept, mastery) {
    const lines = [`Topic: ${concept.name}`];
    if (concept.summary)
    ...
    if (concept.misconceptions.length > 0) {
        lines.push(`Common misconceptions to probe for: ${concept.misconceptions.join("; ")}`);
    }
    ...
    if (concept.sourceRef)
        lines.push(`Source: ${concept.sourceRef}`);
    return lines.join("\n");
}
