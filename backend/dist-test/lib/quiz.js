 ...
 */
export async function buildQuizSession(userId, courseId, opts = {}) {
    const kind = opts.kind ?? "practice";
    const count = Math.min(env.QUIZ_MAX_LENGTH, Math.max(1, opts.count ?? env.QUIZ_DEFAULT_LENGTH));
    ...
    if (all.length === 0) {
        throw new Error("This course has no topics yet — upload some material first.");
    }
    const now = new Date();
    let candidates = all;
    if (opts.topicId) {
    ...
    const questions = await generateQuiz(userId, course.name, allocation.map((a) => ({
        name: a.topic.name,
    ...
    if (env.QUIZ_WRITTEN_COUNT > 0) {
        try {
            written = await generateWrittenQuestions(userId, course.name, allocation.slice(0, 3).map((a) => ({
                name: a.topic.name,
                weight: a.topic.weight,
    ...
    const byName = new Map(candidates.map((t) => [t.name.trim().toLowerCase(), t.id]));
            ...
            items: {
                create: [
                    ...
                    ...written.map((w, i) => ({
                        ...
                        kind: "written",
                        topicId: byName.get(w.topicName.trim().toLowerCase()) ?? fallbackTopicId,
