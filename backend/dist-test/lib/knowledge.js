...
import { AIBudgetExceededError, extractTopics, transcribeImage, } from "./aiMeter.js";
...
function withTimeout(p, ms, label) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error(`${label} took too long — the file may be corrupted.`)), ms);
        p.then((v) => {
            clearTimeout(timer);
            resolve(v);
        }, (e) => {
...
function reinforce(existing, incoming) {
...
function mergeList(existingJson, incoming, cap) {
    let existing = [];
    try {
...
export async function processUpload(uploadId, userId) {
    ...
    try {
        ...
        if (imageKind) {
            ...
            if (text.trim().length === 0) {
                const message = "Couldn't read any study content from this image — try a clearer photo.";
                await prisma.upload.update({
                    where: { id: uploadId },
            ...
            text = await withTimeout(extractText(buffer, upload.filename, upload.mimeType), PARSE_TIMEOUT_MS, "Reading this file");
        }
    }
    catch (err) {
    ...
    let extracted;
    try {
        extracted = await extractTopics(userId, upload.course.name, text, documentInput);
    }
    catch (err) {
        const message = err instanceof Error ? err.message : "Topic extraction failed";
        await prisma.upload.update({
            where: { id: uploadId },
            data: { status: "failed", error: message },
        });
        ...
        if (match) {
            await prisma.topic.update({
                ...
                data: {
                    ...
                    misconceptionsJson: mergeList(match.misconceptionsJson, t.misconceptions ?? [], 5),
                    prerequisitesJson: mergeList(match.prerequisitesJson, t.prerequisites ?? [], 5),
